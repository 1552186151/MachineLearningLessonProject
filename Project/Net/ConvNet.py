...
class Inception(nn.Module):
    def __init__(self,in_c,c1,c2,c3,out_C): # (4, 6, 6, 6, 18)
        super(Inception,self).__init__()
        self.p1 = nn.Sequential(
            nn.Conv2d(in_c, c1,kernel_size=1,padding=0),
            nn.Conv2d(c1, c1, kernel_size=3, padding=1)
        )
        self.p2 = nn.Sequential(
            nn.Conv2d(in_c, c2,kernel_size=1,padding=0),
            nn.Conv2d(c2, c2, kernel_size=5, padding=2)
        )
        self.p3 = nn.Sequential(
            nn.MaxPool2d(kernel_size=3,stride=1,padding=1),
            nn.Conv2d(in_c, c3,kernel_size=3,padding=1),
        )
        self.conv_linear = nn.Conv2d((c1+c2+c3), out_C, 1, 1, 0, bias=True)
...
class DeepSpectra(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.Conv2d(3, 4, kernel_size = 5, stride = 3, padding = 0)
        )
        # self.Inception = Inception(16, 32, 32, 32, 96)
        # self.Inception = Inception(8, 16, 16, 16, 48)
        self.Inception = Inception(4, 6, 6, 6, 18)
