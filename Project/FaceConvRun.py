...
from Project.Net.ConvNet import DeepSpectra
...
def modeltrian(image_size, ncls, psize, depth, heads, dim, mlp_dim, path, data_train, data_test):
    ...
    train_result_path = 'Result/Train/DeepSpectra.csv'
    test_result_path = 'Result/Test/DeepSpectra.csv'
    ...
    plotloss(x = range(train_sum), y1 = avg_train_loss, picd_path = ".//Result//Train//DeepSpectra-loss" + '.png',
             label = "Train_Loss_list")
    plotloss(x = range(train_sum), y1 = avg_test_loss, picd_path = ".//Result//Test//DeepSpectra-loss" + '.png',
             label = "Test_Loss_list")
    plotloss(x = range(train_sum), y1 = avg_train_loss, y2 = avg_test_loss,
             picd_path = ".//Result//DeepSpectra-loss" + '.png')
...
class ConfusionMatrix(object):
    ...
    def plot(self):  # 绘制混淆矩阵
        ...
        plt.tight_layout()
        plt.savefig(".//Result//DeepSpectra-matrix.png")
        plt.show()
...
def model4AUCtest(image_size, ncls, psize, depth, heads, dim, mlp_dim, path, data_test, txt_path):
    ...
    for i, data in enumerate(test_loader):
        ...
        plotROC(false_positive_rate, true_positive_rate, roc_auc, ".//Result//DeepSpectra-roc.png")
    ...
    test_time = endTime - startTime
    date = datetime.datetime.now().strftime('_%Y-%m-%d_%H-%M-%S')  # 不能使用冒号
    plotShow(id_to_species,test_face_dataloader,label_batch = pre_list,path = ".//Result//DeepSpectra-predict.png")
...
if __name__ == "__main__":
    ...
    store_path = 'model/DeepSpectra.pt'
    txt_path = 'Result/DeepSpectra.txt'
